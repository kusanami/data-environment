...
if ! $(hadoop fs -test -d /user/hive/warehouse/matches); then
  ...
  sqoop import --connect jdbc:postgresql://10.60.227.13:5432/maf --table matches --username maf --password MAF_uaDYjiiILdG --query "SELECT * FROM maf.maf_transaccion WHERE fecha_creacion > '2018-10-20'" --create-hive-table --hive-import -m1
fi
